...
import { MinimaxChatProvider, MINIMAX_CHAT_MODELS } from "../providers/minimax.ts";
import type { ChatRequest } from "../types.ts";

const VITE_API_KEY_MINIMAX = import.meta.env.VITE_API_KEY_MINIMAX as string | undefined;

function createProvider() {
  if (!VITE_API_KEY_MINIMAX) return null;
  return new MinimaxChatProvider({ apiKey: VITE_API_KEY_MINIMAX });
}

const MODEL = MINIMAX_CHAT_MODELS[0].value;
...
describe("Minimax API", () => {
  it("non-streaming chat should return a valid response", async () => {
    ...
    if (!provider) {
      console.warn("VITE_API_KEY_MINIMAX is not set, skipping test.");
      return;
    }
  ...
  it("invalid API key should throw authentication error", async () => {
    const provider = new MinimaxChatProvider({ apiKey: "invalid-key-12345" });
